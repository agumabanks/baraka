...
const BranchManagersIndex: React.FC = () => {
  ...
  const activeManagers = managers.filter((m: BranchManager) => m.status === 'active').length;
  const totalBalance = managers.reduce((sum: number, m: BranchManager) => sum + m.current_balance, 0);
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-mono-black mt-2">{totalManagers}</p>
            </div>
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-green-600 mt-2">{activeManagers}</p>
            </div>
            <div className="h-12 w-12 rounded-full bg-green-100 flex items-center justify-center">
              <i className="fas fa-check-circle text-green-600 text-xl" aria-hidden="true" />
            </div>
          </div>
        </Card>
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-mono-black mt-2 font-mono">
                ${totalBalance.toFixed(2)}
              </p>
            </div>
            <div className="h-12 w-12 rounded-full bg-yellow-100 flex items-center justify-center">
              <i className="fas fa-wallet text-yellow-600 text-xl" aria-hidden="true" />
            </div>
          </div>
        </Card>
      </div>
        ...
        <header className="flex flex-col gap-6 border-b border-mono-gray-200 px-8 py-10 xl:flex-row xl:items-center xl:justify-between">
          <div className="space-y-3">
            ...
            <p className="max-w-2xl text-sm leading-relaxed text-mono-gray-600">
              Manage branch managers, monitor performance, track settlements, and oversee client relationships across your network.
            </p>
          </div>
        ...
        <div className="border-b border-mono-gray-200 px-8 py-6">
          <div className="flex flex-wrap gap-4">
            <div className="flex-1 min-w-[250px]">
              <input
                type="text"
                placeholder="Search managers, branches, or business names..."
            ...
            <Card className="text-center">
              <div className="space-y-3">
                <div className="inline-flex h-16 w-16 items-center justify-center rounded-full bg-mono-gray-100 text-mono-gray-600 mb-4">
                  <i className="fas fa-user-tie text-2xl" aria-hidden="true" />
                </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-mono-gray-200 text-left text-xs font-semibold uppercase tracking-[0.25em] text-mono-gray-500">
                    ...
                    <th className="pb-4">COD Charges</th>
                ...
                <tbody>
                  {managers.map((manager: BranchManager) => (
                    ...
                    >
                      ...
                      <td className="py-4">
                        <p className="font-medium text-mono-black">{manager.business_name}</p>
                      </td>
                      <td className="py-4">
                        <div>
                      ...
                      <td className="py-4">
                        {manager.cod_charges && Object.keys(manager.cod_charges).length > 0 ? (
                          <Badge variant="outline" size="sm">
                            Configured
                          </Badge>
                        ) : (
                          <span className="text-sm text-mono-gray-500">Not set</span>
                        )}
                      </td>
                      <td className="py-4">
                      ...
                      <td className="py-4 text-right">
                        <div className="flex justify-end gap-2">
                          ...
                          <Button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              handleDelete(manager.id, manager.user?.name || manager.business_name);
                            }}
