...
return new class extends Migration
{
    ...
    {
        // Customer Promotion Usage Table (only if not already created)
        if (!Schema::hasTable('customer_promotion_usage')) {
        ...
        if (!Schema::hasTable('promotion_effectiveness_metrics')) {
        ...
        if (!Schema::hasTable('customer_milestone_notifications')) {
        ...
        if (!Schema::hasTable('customer_promotion_preferences')) {
        ...
        if (!Schema::hasTable('promotion_ab_tests')) {
        ...
        if (!Schema::hasTable('promotion_notifications_log')) {
        ...
        if (Schema::hasTable('promotional_campaigns')) {
            Schema::table('promotional_campaigns', function (Blueprint $table) {
                if (!$this->indexExists('promotional_campaigns', 'promotional_campaigns_active_period_idx')) {
        ...
        if (Schema::hasTable('customer_promotion_usage')) {
            Schema::table('customer_promotion_usage', function (Blueprint $table) {
                if (!$this->indexExists('customer_promotion_usage', 'usage_customer_campaign_date_idx')) {
                    $table->index(['customer_id', 'promotional_campaign_id', 'used_at'], 'usage_customer_campaign_date_idx');
                }

                if (Schema::hasColumn('customer_promotion_usage', 'promo_code')
                    && !$this->indexExists('customer_promotion_usage', 'usage_code_date_idx')) {
    ...
    public function down(): void
    {
        ...
        if (Schema::hasTable('customer_promotion_usage')) {
            Schema::table('customer_promotion_usage', function (Blueprint $table) {
                ...
                if ($this->indexExists('customer_promotion_usage', 'usage_code_date_idx')) {
