<?php
/**
 * Baraka Logistics Platform - End-to-End System Simulation (Corrected Version)
 * 
 ...
 * and analytics systems using the correct database schema.
 */
...
class BarakaSystemSimulation
{
    ...
    private function checkSystemHealth(): void
    {
        ...
        try {
            ...
            $this->log("Data counts - Shipments: $shipmentsCount, Branches: $branchesCount, Customers: $customersCount");
    ...
    private function simulateBookingPhase(): array
    {
        ...
        try {
            ...
            if (DB::table('edi_transactions')->exists()) {
                $edi850Data = [
                    ...
                    'document_content' => json_encode([
                        ...
                        'ship_to' => '456 Commerce St, Los Angeles, CA 90210, USA',
                        'ship_from' => '123 Business Ave, New York, NY 10001, USA'
                    ]),
                    'status' => 'sent',
                    'created_at' => Carbon::now()
                ];
            ...
            if (DB::table('webhook_endpoints')->exists()) {
                $webhookData = [
    ...
    private function simulateBranchProcessing(array $bookingResult): array
    {
        ...
        try {
            ...
            // Create workflow task if workflow_tasks table exists
            if (DB::table('workflow_tasks')->exists()) {
    ...
    private function simulateInterBranchTransfer(array $branchResult): array
    {
        ...
        try {
            ...
            foreach ($checkpoints as $checkpoint) {
                DB::table('scan_events')->insert([
                    ...
                    'scan_data' => json_encode([
                        ...
                        'operator' => 'Hub Staff',
                        'timestamp' => Carbon::now()->addMinutes(rand(30, 180))->toISOString()
                    ]),
                    'created_at' => Carbon::now()
                ]);
            }
            ...
            // Create transport leg record if transport_legs table exists
            if (DB::table('transport_legs')->exists()) {
            ...
            // Create EDI 856 transaction (Advance Ship Notice) if edi_transactions table exists
            if (DB::table('edi_transactions')->exists()) {
    ...
    private function simulateDeliveryProcess(array $transferResult): array
    {
        ...
        try {
            ...
            // Create EDI 997 transaction (Functional ACK) if edi_transactions table exists
            if (DB::table('edi_transactions')->exists()) {
                DB::table('edi_transactions')->insert([
    ...
    private function testWebhookNotifications(array $bookingResult): array
    {
        ...
        try {
            ...
            if (!DB::table('webhook_endpoints')->exists()) {
                $this->log("Webhook tables do not exist, creating sample data...");
    ...
    private function testEdiProcessing(array $bookingResult): array
    {
        ...
        try {
            // Check if EDI transactions exist for this shipment
    ...
    private function validateAnalyticsSystems(): array
    {
        ...
        try {
            ...
            // Test webhook delivery analytics if webhook_deliveries exists
            ...
            // Test EDI processing analytics if edi_transactions exists
            if (DB::table('edi_transactions')->exists()) {
    ...
    private function testErrorHandling(): array
    {
        ...
        try {
            ...
            // Test webhook failure recovery if webhook_deliveries exists
            if (DB::table('webhook_deliveries')->exists()) {
                try {
