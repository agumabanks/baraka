    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h2 class="fw-bold mb-1">Edit Branch</h2>
                    <p class="text-muted mb-0">Update {{ $branch->name }} details</p>
                </div>
                <div>
    ...
    <div class="row">
        <div class="col-lg-8 col-xl-9">
            <div class="card border-0 shadow-sm">
                <div class="card-body">
                    <form id="branchForm" method="POST" action="{{ route('branches.update', $branch) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                                    ...
                                    @foreach($branchTypes as $type)
                                        <option value="{{ $type }}" {{ old('type', $branch->type) == $type ? 'selected' : '' }}>{{ $type }}</option>
                                    @endforeach
                                </select>
                                @error('type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-select @error('parent_branch_id') is-invalid @enderror" id="parent_branch_id" name="parent_branch_id">
                                    ...
                                    @foreach($potentialParents as $parent)
                                        <option value="{{ $parent->id }}" {{ old('parent_branch_id', $branch->parent_branch_id) == $parent->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-select @error('is_hub') is-invalid @enderror" id="is_hub" name="is_hub">
                                    <option value="0" {{ old('is_hub', $branch->is_hub) == '0' ? 'selected' : '' }}>No</option>
                                    <option value="1" {{ old('is_hub', $branch->is_hub) == '1' ? 'selected' : '' }}>Yes</option>
                                </select>
                                @error('is_hub')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control @error('address') is-invalid @enderror" id="address" name="address" rows="2">{{ old('address', $branch->address) }}</textarea>
                                @error('address')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-2">
                                ...
                                <input type="number" step="0.00000001" class="form-control @error('latitude') is-invalid @enderror" id="latitude" name="latitude" value="{{ old('latitude', $branch->latitude) }}">
                                @error('latitude')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-2">
                                ...
                                <input type="number" step="0.00000001" class="form-control @error('longitude') is-invalid @enderror" id="longitude" name="longitude" value="{{ old('longitude', $branch->longitude) }}">
                                @error('longitude')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                    <option value="{{ App\Enums\Status::ACTIVE }}" {{ old('status', $branch->status) == App\Enums\Status::ACTIVE ? 'selected' : '' }}>Active</option>
                                    <option value="{{ App\Enums\Status::INACTIVE }}" {{ old('status', $branch->status) == App\Enums\Status::INACTIVE ? 'selected' : '' }}>Inactive</option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-body">
                    ...
                    @if($branch->children->count() > 0)
                        <div class="mb-3">
                            <small class="text-muted d-block">Child Branches</small>
                            <p class="mb-0">{{ $branch->children->count() }}</p>
                        </div>
                    @endif
                </div>
            </div>
...
document.getElementById('branchForm').addEventListener('submit', function(e) {
    ...
    .then(data => {
        if (data.success) {
            window.location.href = '{{ route("branches.show", $branch) }}';
        } else {
            alert(data.message || 'Failed to update branch');
            submitBtn.disabled = false;
            submitBtn.innerHTML = '<i class="fas fa-save me-2"></i>Update Branch';
        }
    })
    .catch(error => {
