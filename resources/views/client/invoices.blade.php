...
@section('title', trans_db('client.invoices.title', [], null, 'Invoices'))
@section('header', trans_db('client.invoices.header', [], null, 'Invoices'))
        ...
        @if($invoices->count() > 0)
                            ...
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 rounded-lg bg-amber-500/20 flex items-center justify-center">
	                                ...
	                                <div>
	                                    ...
	                                    <div class="text-sm text-zinc-400">{{ $invoice->created_at->locale(app()->getLocale())->translatedFormat('M d, Y') }}</div>
	                                </div>
	                            </div>
	                            <div class="text-right">
	                                <div class="font-bold">${{ number_format($invoice->total ?? 0, 2) }}</div>
	                                @php
	                                    $invoiceStatusKey = strtolower($invoice->status ?? 'unpaid');
	                                    $invoiceStatusDefault = ucfirst($invoiceStatusKey);
	                                @endphp
	                                <span class="px-2 py-0.5 rounded text-xs {{ ($invoice->status ?? 'unpaid') === 'paid' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400' }}">
	                                    {{ trans_db("client.invoices.status.{$invoiceStatusKey}", [], null, $invoiceStatusDefault) }}
	                                </span>
	                            </div>
	                        </div>
	                    </div>
	        ...
	        @else
	            <div class="p-12 text-center">
	                ...
	                <h3 class="text-lg font-medium mb-2">{{ trans_db('client.invoices.empty.title', [], null, 'No Invoices') }}</h3>
	                <p class="text-zinc-400">{{ trans_db('client.invoices.empty.subtitle', [], null, "You don't have any invoices yet.") }}</p>
	            </div>
	        @endif
	    </div>
@endsection
