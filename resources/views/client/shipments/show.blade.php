...
@section('title', 'Shipment ' . ($shipment->tracking_number ?? $shipment->awb_number))
...
@section('content')
    ...
    <div class="grid lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="glass-panel p-6">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <div class="text-sm text-zinc-400 mb-1">Tracking Number</div>
                    ...
                    <div class="flex justify-between mb-2">
                        ...
                        @foreach($steps as $index => $step)
                            <div class="flex flex-col items-center flex-1">
                                ...
                                <span class="text-xs text-center {{ $index <= $currentIndex ? 'text-white' : 'text-zinc-500' }}">{{ ucfirst(str_replace('_', ' ', $step)) }}</span>
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="glass-panel p-6">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1 p-4 bg-zinc-800/50 rounded-lg">
                        ...
                        <div class="font-medium">{{ $shipment->destinationBranch?->name ?? 'N/A' }}</div>
            ...
            <div class="glass-panel p-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                        <div class="text-xs text-zinc-500 uppercase mb-1">Weight</div>
                        <div class="font-medium">{{ number_format($shipment->weight ?? 0, 2) }} kg</div>
                    </div>
                    <div>
                        <div class="text-xs text-zinc-500 uppercase mb-1">Pieces</div>
                        <div class="font-medium">{{ $shipment->pieces ?? 1 }}</div>
                    </div>
                    <div>
                        <div class="text-xs text-zinc-500 uppercase mb-1">Service</div>
                        <div class="font-medium capitalize">{{ $shipment->service_level ?? 'Standard' }}</div>
                    </div>
                    <div>
                        <div class="text-xs text-zinc-500 uppercase mb-1">Declared Value</div>
                        <div class="font-medium">${{ number_format($shipment->declared_value ?? 0, 2) }}</div>
                    </div>
                </div>
                @if($shipment->description)
                    <div class="mt-4 pt-4 border-t border-white/10">
                        <div class="text-xs text-zinc-500 uppercase mb-1">Description</div>
        ...
        <div class="space-y-6">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="px-2 py-0.5 rounded text-xs {{ ($shipment->payment_status ?? 'unpaid') === 'paid' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400' }}">
                            {{ ucfirst($shipment->payment_status ?? 'Unpaid') }}
                        </span>
                    </div>
                </div>
            </div>
            ...
            {{-- Receiver Info --}}
            <div class="glass-panel p-6">
                <h3 class="font-semibold mb-4">Receiver</h3>
                <div class="space-y-3 text-sm">
                    @php $meta = $shipment->metadata ?? []; @endphp
                    <div>
                        <div class="text-xs text-zinc-500 uppercase mb-1">Name</div>
                        <div>{{ $meta['receiver_name'] ?? 'N/A' }}</div>
                    </div>
                    <div>
                        <div class="text-xs text-zinc-500 uppercase mb-1">Phone</div>
                        <div>{{ $meta['receiver_phone'] ?? 'N/A' }}</div>
                    </div>
                    <div>
                        <div class="text-xs text-zinc-500 uppercase mb-1">Address</div>
                        <div>{{ $meta['delivery_address'] ?? 'N/A' }}</div>
                    </div>
                </div>
            </div>
                    ...
                    @if($shipment->updated_at && $shipment->updated_at != $shipment->created_at)
                        <div class="flex gap-3">
                            ...
                            <div>
                                ...
                                <div class="text-sm text-zinc-400">{{ $shipment->updated_at->format('M d, Y H:i') }}</div>
                            </div>
                        </div>
                    @endif
                </div>
