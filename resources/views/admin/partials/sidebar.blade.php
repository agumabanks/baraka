<aside class="fixed inset-y-0 left-0 z-40 w-64 transform transition-transform duration-300 lg:translate-x-0 bg-obsidian-950 border-r border-white/10 flex flex-col -translate-x-full" data-sidebar>
    <div class="h-16 flex items-center px-6 border-b border-white/10">
        <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3">
            <img src="{{ \App\Support\SystemSettings::adminLogo() }}" alt="{{ \App\Support\SystemSettings::companyName() }}" class="h-8 w-auto">
            <div class="font-bold text-lg tracking-tight">Admin<span class="text-emerald-500">Panel</span></div>
        </a>
    </div>

    <div class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
        <div class="px-3 mb-2 text-xs font-semibold uppercase tracking-wider text-slate-500">{{ trans_db('admin.sidebar.section.overview', [], null, 'Overview') }}</div>
        <a href="{{ route('admin.dashboard') }}" class="nav-link {{ request()->routeIs('admin.dashboard') ? 'nav-link-active' : '' }}">
            ...
            {{ trans_db('admin.sidebar.dashboard', [], null, 'Dashboard') }}
        </a>

        <div class="px-3 mt-6 mb-2 text-xs font-semibold uppercase tracking-wider text-slate-500">{{ trans_db('admin.sidebar.section.operations', [], null, 'Operations') }}</div>
        <a href="{{ route('admin.pos.index') }}" class="nav-link {{ request()->routeIs('admin.pos.*') ? 'nav-link-active' : '' }}">
            ...
            <span class="flex items-center gap-2">{{ trans_db('admin.sidebar.shipments_pos', [], null, 'Shipments POS') }} <span class="text-xs px-1.5 py-0.5 rounded bg-emerald-500/20 text-emerald-400">{{ trans_db('admin.badge.new', [], null, 'NEW') }}</span></span>
        </a>
        <a href="{{ route('admin.shipments.index') }}" class="nav-link {{ request()->routeIs('admin.shipments.*') ? 'nav-link-active' : '' }}">
            ...
            {{ trans_db('admin.sidebar.shipments_management', [], null, 'Shipments Management') }}
        </a>
        <a href="{{ route('admin.branches.index') }}" class="nav-link {{ request()->routeIs('admin.branches.*') ? 'nav-link-active' : '' }}">
            ...
            {{ trans_db('admin.sidebar.branches', [], null, 'Branches') }}
        </a>
        <a href="{{ route('admin.hubs.index') }}" class="nav-link {{ request()->routeIs('admin.hubs.*') ? 'nav-link-active' : '' }}">
            ...
            {{ trans_db('admin.sidebar.hubs', [], null, 'Hubs') }}
        </a>

        <div class="px-3 mt-6 mb-2 text-xs font-semibold uppercase tracking-wider text-slate-500">{{ trans_db('admin.sidebar.section.clients_personnel', [], null, 'Clients & Personnel') }}</div>
        <a href="{{ route('admin.clients.index') }}" class="nav-link {{ request()->routeIs('admin.clients.*') ? 'nav-link-active' : '' }}">
            ...
            {{ trans_db('admin.sidebar.clients', [], null, 'Clients') }}
        </a>
        <a href="{{ route('admin.merchants.index') }}" class="nav-link {{ request()->routeIs('admin.merchants.*') ? 'nav-link-active' : '' }}">
            ...
            {{ trans_db('admin.sidebar.merchants', [], null, 'Merchants') }}
        </a>
        <a href="{{ route('admin.delivery-personnel.index') }}" class="nav-link {{ request()->routeIs('admin.delivery-personnel.*') ? 'nav-link-active' : '' }}">
            ...
            {{ trans_db('admin.sidebar.delivery_personnel', [], null, 'Delivery Personnel') }}
        </a>

        <div class="px-3 mt-6 mb-2 text-xs font-semibold uppercase tracking-wider text-slate-500">{{ trans_db('admin.sidebar.section.tracking_dispatch', [], null, 'Tracking & Dispatch') }}</div>
        <a href="{{ route('admin.tracking.dashboard') }}" class="nav-link {{ request()->routeIs('admin.tracking.*') ? 'nav-link-active' : '' }}">
            ...
            {{ trans_db('admin.sidebar.live_tracking', [], null, 'Live Tracking') }}
        </a>
        <a href="{{ route('admin.dispatch.index') }}" class="nav-link {{ request()->routeIs('admin.dispatch.*') ? 'nav-link-active' : '' }}">
            ...
            {{ trans_db('admin.sidebar.dispatch_center', [], null, 'Dispatch Center') }}
        </a>

        <div class="px-3 mt-6 mb-2 text-xs font-semibold uppercase tracking-wider text-slate-500">{{ trans_db('admin.sidebar.section.finance', [], null, 'Finance') }}</div>
        <a href="{{ route('admin.finance.consolidated') }}" class="nav-link {{ request()->routeIs('admin.finance.consolidated') ? 'nav-link-active' : '' }}">
            ...
            {{ trans_db('admin.sidebar.network_finance', [], null, 'Network Finance') }}
        </a>
        <a href="{{ route('admin.finance.dashboard') }}" class="nav-link {{ request()->routeIs('admin.finance.dashboard') ? 'nav-link-active' : '' }}">
            ...
            {{ trans_db('admin.sidebar.cod_collections', [], null, 'COD & Collections') }}
        </a>
        <a href="{{ route('admin.finance.cod.index') }}" class="nav-link {{ request()->routeIs('admin.finance.cod.*') ? 'nav-link-active' : '' }}">
            ...
            {{ trans_db('admin.sidebar.cod_management', [], null, 'COD Management') }}
        </a>
        <a href="{{ route('admin.finance.settlements.index') }}" class="nav-link {{ request()->routeIs('admin.finance.settlements.*') ? 'nav-link-active' : '' }}">
            ...
            {{ trans_db('admin.sidebar.settlements', [], null, 'Settlements') }}
        </a>

        <div class="px-3 mt-6 mb-2 text-xs font-semibold uppercase tracking-wider text-slate-500">{{ trans_db('admin.sidebar.section.analytics', [], null, 'Analytics') }}</div>
        <a href="{{ route('admin.analytics.dashboard') }}" class="nav-link {{ request()->routeIs('admin.analytics.dashboard') ? 'nav-link-active' : '' }}">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>
            {{ trans_db('admin.sidebar.executive_dashboard', [], null, 'Executive Dashboard') }}
        </a>
        <a href="{{ route('admin.analytics.reports') }}" class="nav-link {{ request()->routeIs('admin.analytics.reports') ? 'nav-link-active' : '' }}">
            ...
            {{ trans_db('admin.sidebar.analytics_reports', [], null, 'Reports') }}
        </a>

        <div class="px-3 mt-6 mb-2 text-xs font-semibold uppercase tracking-wider text-slate-500">{{ trans_db('admin.sidebar.section.security', [], null, 'Security') }}</div>
        <a href="{{ route('admin.security.dashboard') }}" class="nav-link {{ request()->routeIs('admin.security.dashboard') ? 'nav-link-active' : '' }}">
            ...
            {{ trans_db('admin.sidebar.security_center', [], null, 'Security Center') }}
        </a>
        <a href="{{ route('admin.security.mfa') }}" class="nav-link {{ request()->routeIs('admin.security.mfa*') ? 'nav-link-active' : '' }}">
            ...
            {{ trans_db('admin.sidebar.two_factor_auth', [], null, 'Two-Factor Auth') }}
        </a>
        <a href="{{ route('admin.security.mfa-settings') }}" class="nav-link {{ request()->routeIs('admin.security.mfa-settings') ? 'nav-link-active' : '' }}">
            ...
            {{ trans_db('admin.sidebar.mfa_policy_settings', [], null, 'MFA Policy Settings') }}
        </a>

        <div class="px-3 mt-6 mb-2 text-xs font-semibold uppercase tracking-wider text-slate-500">{{ trans_db('admin.sidebar.section.management', [], null, 'Management') }}</div>
        <a href="{{ route('admin.users.index') }}" class="nav-link {{ request()->routeIs('admin.users.*') ? 'nav-link-active' : '' }}">
            ...
            {{ trans_db('admin.sidebar.users', [], null, 'Users') }}
        </a>
        <a href="{{ route('admin.reports') }}" class="nav-link {{ request()->routeIs('admin.reports') ? 'nav-link-active' : '' }}">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
            {{ trans_db('admin.sidebar.reports', [], null, 'Reports') }}
        </a>

        <div class="px-3 mt-6 mb-2 text-xs font-semibold uppercase tracking-wider text-slate-500">{{ trans_db('admin.sidebar.section.settings', [], null, 'Settings') }}</div>
        <a href="{{ route('settings.index') }}" class="nav-link {{ request()->routeIs('settings.*') ? 'nav-link-active' : '' }}">
            ...
            {{ trans_db('admin.sidebar.settings', [], null, 'Settings') }}
        </a>
        <a href="{{ route('general-settings.index') }}" class="nav-link {{ request()->routeIs('general-settings.*') ? 'nav-link-active' : '' }}">
            ...
            {{ trans_db('admin.sidebar.general_settings', [], null, 'General Settings') }}
        </a>
    </div>
            ...
            <div class="absolute bottom-full left-0 w-full mb-2 bg-obsidian-900 border border-white/10 rounded-lg shadow-xl overflow-hidden hidden" data-user-menu>
                <form method="POST" action="{{ route('logout') }}">
                    ...
                    <button type="submit" class="w-full text-left px-4 py-2 text-sm text-rose-400 hover:bg-white/5 transition flex items-center gap-2">
                        ...
                        {{ trans_db('admin.sidebar.sign_out', [], null, 'Sign Out') }}
                    </button>
                </form>
            </div>
        </div>
