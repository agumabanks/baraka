        ...
        <div class="flex items-center gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-zinc-500">{{ $currentBranch->name ?? 'All Branches' }} &bull; {{ now()->format('D, M d Y') }}</p>
                </div>
            </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        <div class="xl:col-span-8 space-y-5">
            ...
            <div class="pos-card p-5">
                ...
                <div class="mb-4">
                    ...
                    <div id="packagePresetsContainer" class="flex flex-wrap gap-2">
        ...
        <div class="xl:col-span-4 space-y-5">
            ...
            <div class="pos-card p-5 sticky top-4">
                ...
                {{-- Payment Method (populated from system settings) --}}
                <div class="mb-4" id="paymentSection">
                    ...
                    <div id="paymentMethodsGrid" class="grid grid-cols-2 gap-2">
                        {{-- Populated by JavaScript from system settings --}}
                ...
                <div class="space-y-1 max-h-64 overflow-y-auto">
                    ...
                    <div class="shipment-row flex items-center justify-between">
                        <div>
                            ...
                            <div class="text-xs text-zinc-500">{{ $shipment->customer?->name ?? 'N/A' }} &bull; {{ $shipment->created_at->diffForHumans() }}</div>
                        </div>
                        <div class="text-right">
                            ...
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.pos.label', $shipment) }}" target="_blank" class="text-xs text-blue-400 hover:text-blue-300">Label</a>
                                <a href="{{ route('admin.pos.receipt', $shipment) }}" target="_blank" class="text-xs text-blue-400 hover:text-blue-300">Receipt</a>
                            </div>
                        </div>
                    </div>
                    @empty
...
let holdQueue = JSON.parse(localStorage.getItem('posHoldQueue') || '[]');
let searchResults = [];
let debounceTimer = null;
let lastTransaction = JSON.parse(localStorage.getItem('posLastTransaction') || 'null');
...
function playSound(type) {
    // Sound feedback placeholder - can implement Web Audio API
...
function showLoading() {
    document.getElementById('sanaaLoadingOverlay').classList.remove('hidden');
}

function hideLoading() {
    document.getElementById('sanaaLoadingOverlay').classList.add('hidden');
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => {
        hideLoading();
        playSound('ready');
        console.log('POS Terminal Ready - System Config:', systemConfig);
    }, 800); // Small delay for smooth transition
});

// Hide loading on window load (fallback)
window.addEventListener('load', function() {
    setTimeout(hideLoading, 1000);
});

// ============================================
// FORM LISTENERS (Auto-sync everything)
// ============================================
function initFormListeners() {
    console.log('Initializing form listeners...');
    
    // Weight changes - both input and change events
    ...
    // Value and option changes
    ['declaredValue', 'codAmount', 'pieces'].forEach(id => {
        const el = document.getElementById(id);
        if (el) {
            el.addEventListener('input', scheduleRateCalculation);
            el.addEventListener('change', scheduleRateCalculation);
        }
    });
    ...
    // Destination branch change - most important trigger
    const destBranch = document.getElementById('destBranch');
    if (destBranch) {
        destBranch.addEventListener('change', () => {
            console.log('Destination changed:', destBranch.value);
            scheduleRateCalculation();
            fetchServicePricing();
            updateValidationState();
        });
    ...
    // Origin branch change
    const originBranch = document.getElementById('originBranch');
    if (originBranch) {
        originBranch.addEventListener('change', () => {
            console.log('Origin changed:', originBranch.value);
            scheduleRateCalculation();
        });
    }
...
function scheduleRateCalculation() {
    clearTimeout(rateCalcTimer);
    rateCalcTimer = setTimeout(() => {
        console.log('Rate calculation scheduled');
        calculateRate();
    }, 300);
}
...
function initPackagePresets() {
    ...
    container.innerHTML = systemConfig.packagePresets.map((p, i) => `
        <button type="button" onclick="applyPackagePreset(${i})" class="quick-action text-xs">
            ${p.name}
        </button>
    `).join('');
}
...
function initPaymentMethods() {
    ...
    container.innerHTML = methods
        .filter(m => systemConfig.paymentMethods[m.key])
        .map(m => `
            <button type="button" onclick="selectPayment('${m.key}')" class="payment-option" data-method="${m.key}">
                <svg class="w-6 h-6 mx-auto mb-1 text-${m.color}-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ${getPaymentIcon(m.key)}
                </svg>
...
function initCustomerSearch() {
    ...
    resultsDiv.addEventListener('click', function(e) {
        ...
        if (item && searchResults[item.dataset.index]) {
            selectCustomer(searchResults[item.dataset.index]);
        }
    });
}
...
async function searchCustomer(query) {
    try {
        const response = await fetch(`{{ route('admin.pos.search-customer') }}?q=${encodeURIComponent(query)}`);
        ...
        if (searchResults.length === 0) {
            ...
            resultsDiv.innerHTML = searchResults.map((c, i) => `
                <div class="customer-result" data-index="${i}">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 bg-zinc-700 rounded-full flex items-center justify-center text-sm font-bold text-white">
...
function updateValidationState() {
    ...
    // Visual indicators
    document.getElementById('customerSearch')?.parentElement?.classList.toggle('ring-2', !hasCustomer);
    document.getElementById('customerSearch')?.parentElement?.classList.toggle('ring-red-500/50', !hasCustomer);
...
function updateVolumetricWeight() {
    ...
    // Volumetric weight formula: L x W x H / 5000 (for cm and kg)
    ...
    // Highlight if volumetric is higher
    const chargeEl = document.getElementById('chargeWeight');
    if (volWeight > actualWeight && volWeight > 0) {
        chargeEl.classList.add('text-yellow-400');
        chargeEl.classList.remove('text-green-400');
    } else {
        chargeEl.classList.remove('text-yellow-400');
        chargeEl.classList.add('text-green-400');
    }
...
function selectService(service) {
    ...
    // Show service info
    const serviceInfo = systemConfig.serviceLevels[service];
    if (serviceInfo) {
        showToast(`${serviceInfo.name}: ${serviceInfo.days} days`, 'info');
    }
...
async function calculateRate() {
    ...
    // Need destination and weight to calculate
    if (!destBranch || weight <= 0) {
        console.log('Rate calc skipped: destBranch=', destBranch, 'weight=', weight);
        return;
    }
    
    isCalculating = true;
    console.log('Calculating rate...', { originBranch, destBranch, serviceLevel, weight });
    
    try {
        const response = await fetch('{{ route('admin.pos.calculate-rate') }}', {
            method: 'POST',
...
async function fetchServicePricing() {
    ...
    try {
        const response = await fetch(`{{ route('admin.pos.service-levels') }}?origin_branch_id=${originBranch}&dest_branch_id=${destBranch}&weight=${weight}`);
        const data = await response.json();
        ...
        if (data.success && data.service_levels) {
            data.service_levels.forEach(level => {
                ...
                if (priceEl) {
                    priceEl.textContent = '$' + (level.total || 0).toFixed(2);
                }
            });
        }
    } catch (error) {
...
async function createShipment() {
    ...
    if (!destBranch) {
        ...
        document.getElementById('destBranch').focus();
        return;
    }
    ...
    try {
        const response = await fetch('{{ route('admin.pos.create-shipment') }}', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
            body: JSON.stringify({
        ...
        if (data.success) {
            playSound('success');
            showSuccessModal(data.shipment, data.urls);
        } else {
            playSound('error');
...
function showSuccessModal(shipment, urls) {
    // Save last transaction for repeat feature
    lastTransaction = {
    ...
    localStorage.setItem('posLastTransaction', JSON.stringify(lastTransaction));
...
function holdTransaction() {
    ...
    localStorage.setItem('posHoldQueue', JSON.stringify(holdQueue));
    updateHoldCount();
    playSound('hold');
...
function updateHoldCount() {
    document.getElementById('holdCount').textContent = holdQueue.length;
}

function showHoldQueue() {
    if (holdQueue.length === 0) {
        showToast('No held transactions', 'info');
        return;
    }
        ...
        html += `
            <div class="bg-zinc-800 rounded-lg p-4">
                <div class="flex justify-between items-start mb-2">
                    <div>
                        <div class="font-semibold text-white">${escapeHtml(t.customer?.name || 'Unknown')}</div>
                        <div class="text-xs text-zinc-500">${new Date(t.timestamp).toLocaleString()}</div>
                    </div>
                    <div class="text-right">
                        <div class="text-green-400 font-semibold">${systemConfig.currencySymbol}${(t.pricing?.total || 0).toFixed(2)}</div>
                        <div class="text-xs text-zinc-500">${t.weight || 0} kg</div>
                    </div>
                </div>
                <div class="flex gap-2">
                    <button onclick="restoreHeld(${i})" class="flex-1 py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm rounded-lg">Restore</button>
                    <button onclick="removeHeld(${i})" class="px-4 py-2 bg-red-600/20 hover:bg-red-600/40 text-red-400 text-sm rounded-lg">Remove</button>
                </div>
            </div>
        `;
    });
...
function restoreHeld(index) {
    ...
    if (t.pricing) {
        currentPricing = t.pricing;
        updatePricingDisplay(t.pricing);
    }
...
function showRepeatLast() {
    if (!lastTransaction) {
        showToast('No previous transaction to repeat', 'info');
        return;
    }
    
    // Fill form with last transaction data (except customer)
...
async function quickTrack() {
    ...
    try {
        const response = await fetch(`{{ route('admin.pos.quick-track') }}?tracking=${encodeURIComponent(tracking)}`);
        const data = await response.json();
        
        if (data.success) {
            showTrackResult(data.shipment);
        } else {
            showToast('Shipment not found', 'error');
        }
    } catch (error) {
        showToast('Track failed', 'error');
    }
}
...
async function createCustomer() {
    ...
    try {
        const response = await fetch('{{ route('admin.pos.quick-create-customer') }}', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
            body: JSON.stringify({
                name: name,
...
function viewCustomerHistory() {
    showToast('Customer history coming soon', 'info');
}
