...
@section('header', 'Create New Branch')
        ...
        <div class="glass-panel p-5 space-y-4">
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code') }}" class="w-full bg-obsidian-800 border border-white/10 rounded px-3 py-2" required>
                </div>
            </div>
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <select name="type" class="w-full bg-obsidian-800 border border-white/10 rounded px-3 py-2">
                        ...
                        <option value="hub" @selected(old('type') === 'hub')>Hub</option>
                        <option value="regional" @selected(old('type') === 'regional')>Regional</option>
                    </select>
                </div>
                <div>
                    ...
                    <select name="parent_branch_id" class="w-full bg-obsidian-800 border border-white/10 rounded px-3 py-2">
                        ...
                        @foreach($branches as $hub)
                            <option value="{{ $hub->id }}" @selected(old('parent_branch_id') == $hub->id)>{{ $hub->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" class="w-full bg-obsidian-800 border border-white/10 rounded px-3 py-2">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-200 mb-1">Phone</label>
                    <input type="text" name="phone" value="{{ old('phone') }}" class="w-full bg-obsidian-800 border border-white/10 rounded px-3 py-2">
                </div>
            </div>

            <div>
                <label class="block text-sm font-medium text-slate-200 mb-1">Address</label>
                <textarea name="address" rows="3" class="w-full bg-obsidian-800 border border-white/10 rounded px-3 py-2">{{ old('address') }}</textarea>
            </div>

            <div class="flex items-center gap-3">
                <input type="checkbox" id="is_hub" name="is_hub" value="1" class="rounded border-white/20 bg-obsidian-800" @checked(old('is_hub'))>
