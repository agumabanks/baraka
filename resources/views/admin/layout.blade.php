...
<head>
    ...
    <title>@yield('title', trans_db('admin.layout.title', [], null, 'Admin Panel')) • {{ config('app.name') }}</title>
            ...
            <header class="sticky top-0 z-30 border-b border-white/10 bg-obsidian-800/80 backdrop-blur">
                <div class="px-4 lg:px-8 py-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    <div class="flex items-center gap-3">
                        <button class="lg:hidden nav-link !px-2 !py-1" type="button" data-sidebar-toggle>{{ trans_db('admin.layout.menu', [], null, 'Menu') }}</button>
                        <div>
                            <div class="text-xs uppercase muted">{{ trans_db('admin.layout.system_administration', [], null, 'System Administration') }}</div>
                            <div class="text-xl font-semibold leading-tight">@yield('header', trans_db('admin.layout.header.default', [], null, 'Dashboard'))</div>
                        </div>
                    </div>
                    <div class="flex flex-wrap items-center gap-3">
                        <x-language-switcher style="dropdown" :show-flags="true" :show-labels="true" />
                        <div class="text-right">
                            <div class="muted text-2xs">{{ trans_db('admin.layout.signed_in_as', [], null, 'Signed in as') }}</div>
            ...
            <main class="flex-1 px-4 lg:px-8 py-6 space-y-6">
                ...
                @if($errors->any())
                    <div class="glass-panel px-4 py-3 border-amber-500/30 text-amber-100">
                        <div class="font-semibold">{{ trans_db('admin.layout.errors.fix_following', [], null, 'Please fix the following:') }}</div>
