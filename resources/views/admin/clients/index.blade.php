...
@section('content')
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-sm muted">Manage customers, contracts, and credit limits</p>
        </div>
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('admin.clients.create') }}" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 rounded-lg font-medium transition-colors flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                </svg>
                New Client
            </a>
            <a href="{{ route('admin.clients.export', request()->query()) }}" class="px-4 py-2 bg-zinc-700 hover:bg-zinc-600 rounded-lg font-medium transition-colors flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="glass-panel p-5">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-4">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <select id="branchFilter" class="bg-obsidian-700 border border-white/10 rounded px-3 py-2 text-sm">
        ...
        <div id="bulkActionsBar" class="hidden bg-zinc-800/50 border border-white/10 rounded-lg p-3 mb-4">
            <form method="POST" action="{{ route('admin.clients.bulk-action') }}" class="flex flex-wrap items-center gap-3">
                @csrf
                ...
                <select name="branch_id" id="bulkBranchSelect" class="bg-obsidian-700 border border-white/10 rounded px-3 py-1.5 text-sm hidden">
                    @foreach($branches as $branch)
        ...
        <div class="overflow-x-auto rounded-lg border border-white/10">
            <table class="w-full">
                ...
                <tbody id="clientsTableBody" class="divide-y divide-white/5">
                    @include('admin.clients._table', ['customers' => $customers])
                </tbody>
            </table>
        </div>
        ...
        <div id="paginationContainer" class="mt-4">
            @include('admin.clients._pagination', ['customers' => $customers, 'perPage' => $perPage ?? 10])
        </div>
    </div>
@endsection
...
function performSearch() {
    ...
    .then(data => {
        ...
        window.history.replaceState({}, '', `{{ route('admin.clients.index') }}?${params.toString()}`);
    })
...
['branchFilter', 'statusFilter', 'typeFilter'].forEach(id => {
...
document.querySelector('select[name="action"]')?.addEventListener('change', function() {
    const branchSelect = document.getElementById('bulkBranchSelect');
    branchSelect.classList.toggle('hidden', this.value !== 'assign_branch');
});
