...
@section('content')
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div class="flex items-center gap-4">
            <a href="{{ route('admin.clients.index') }}" class="p-2 rounded-lg hover:bg-white/10 transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
                </svg>
            </a>
            <div class="flex items-center gap-3">
                <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-emerald-500/30 to-blue-500/30 flex items-center justify-center text-lg font-bold">
        ...
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('admin.clients.quick-shipment', $client) }}" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 rounded-lg font-medium transition-colors flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                </svg>
                New Shipment
            </a>
            <a href="{{ route('admin.clients.statement', $client) }}" class="px-4 py-2 bg-zinc-700 hover:bg-zinc-600 rounded-lg font-medium transition-colors flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
                Statement
            </a>
            <a href="{{ route('admin.clients.edit', $client) }}" class="px-4 py-2 bg-zinc-700 hover:bg-zinc-600 rounded-lg font-medium transition-colors flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        ...
        <div class="space-y-6">
            ...
            <div class="glass-panel p-5">
                ...
                <form method="POST" action="{{ route('admin.clients.activity.store', $client) }}" class="space-y-3">
                    @csrf
            ...
            <div class="glass-panel p-5">
                ...
                <form method="POST" action="{{ route('admin.clients.reminder.store', $client) }}" class="space-y-3">
                    @csrf
                        ...
                        @foreach($client->crmReminders as $reminder)
                            <div class="border border-white/10 rounded p-3">
                                <div class="flex items-start justify-between">
                                    ...
                                    <form method="POST" action="{{ route('admin.clients.reminder.complete', [$client, $reminder]) }}">
                                        @csrf
    ...
    <div id="creditModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50" onclick="if(event.target === this) this.classList.add('hidden')">
        <div class="glass-panel p-6 w-full max-w-md mx-4">
            ...
            <form method="POST" action="{{ route('admin.clients.adjust-credit', $client) }}" class="space-y-4">
                @csrf
                <div>
