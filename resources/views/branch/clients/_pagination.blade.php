<div class="flex flex-col sm:flex-row items-center justify-between gap-4 px-4 py-3 bg-zinc-800/50 rounded-lg border border-white/10">
    <div class="flex items-center gap-4">
        <div class="text-sm text-zinc-400">
            @if($customers->total() > 0)
                Showing {{ $customers->firstItem() }} to {{ $customers->lastItem() }} of {{ $customers->total() }} clients
            @else
                No clients found
            @endif
        </div>
        <div class="flex items-center gap-2">
            <span class="text-sm text-zinc-500">Show:</span>
            <select id="perPageSelect" onchange="changePerPage(this.value)" class="bg-zinc-700 border border-white/10 rounded px-2 py-1 text-sm focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500">
                <option value="10" @selected(($perPage ?? 10) == 10)>10</option>
                <option value="25" @selected(($perPage ?? 10) == 25)>25</option>
                <option value="50" @selected(($perPage ?? 10) == 50)>50</option>
                <option value="100" @selected(($perPage ?? 10) == 100)>100</option>
            </select>
        </div>
    </div>
    
    @if($customers->lastPage() > 1)
        <div class="flex items-center gap-2">
            @if($customers->onFirstPage())
                <span class="px-3 py-1.5 text-sm text-zinc-600 cursor-not-allowed">Previous</span>
            @else
                <a href="{{ $customers->previousPageUrl() }}&per_page={{ $perPage ?? 10 }}" class="pagination-link px-3 py-1.5 text-sm bg-zinc-700 hover:bg-zinc-600 rounded-lg transition-colors">Previous</a>
            @endif
            
            <div class="flex items-center gap-1">
                @foreach($customers->getUrlRange(max(1, $customers->currentPage() - 2), min($customers->lastPage(), $customers->currentPage() + 2)) as $page => $url)
                    @if($page == $customers->currentPage())
                        <span class="px-3 py-1.5 text-sm bg-emerald-600 rounded-lg">{{ $page }}</span>
                    @else
                        <a href="{{ $url }}&per_page={{ $perPage ?? 10 }}" class="pagination-link px-3 py-1.5 text-sm bg-zinc-700 hover:bg-zinc-600 rounded-lg transition-colors">{{ $page }}</a>
                    @endif
                @endforeach
            </div>
            
            @if($customers->hasMorePages())
                <a href="{{ $customers->nextPageUrl() }}&per_page={{ $perPage ?? 10 }}" class="pagination-link px-3 py-1.5 text-sm bg-zinc-700 hover:bg-zinc-600 rounded-lg transition-colors">Next</a>
            @else
                <span class="px-3 py-1.5 text-sm text-zinc-600 cursor-not-allowed">Next</span>
            @endif
        </div>
    @endif
</div>
