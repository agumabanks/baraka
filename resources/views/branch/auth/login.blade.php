...
<head>
    ...
    <title>Branch Access • {{ config('app.name') }}</title>
    ...
    <div class="relative z-10 flex h-full w-full items-center justify-center p-4">
        <div class="w-full max-w-[420px] animate-slide-up">
            ...
            <div class="glass-card rounded-2xl p-8 shadow-2xl shadow-black/50">
                ...
                <form method="POST" action="{{ route('branch.login.submit') }}" class="space-y-5">
                    @csrf
                    ...
                    <div class="space-y-1.5 input-group transition-colors duration-200">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="login" id="login" required autofocus
                                ...
                                placeholder="name@company.com or +2567..."
                                value="{{ old('login') }}">
                        </div>
                        @error('login')
                    ...
                    <div class="flex items-center justify-between pt-2">
                        <label class="flex items-center gap-2 cursor-pointer group">
                            <input type="checkbox" name="remember" class="rounded border-white/10 bg-gray-900/50 text-emerald-500 focus:ring-emerald-500/50 transition-colors group-hover:border-emerald-500/50">
                            <span class="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">Remember me</span>
                        </label>
                        <a href="#" class="text-sm font-medium text-emerald-500 hover:text-emerald-400 transition-colors">Forgot password?</a>
                    </div>

                    <button type="submit" class="group relative flex w-full justify-center rounded-lg bg-emerald-600 px-4 py-3 text-sm font-semibold text-white shadow-lg transition-all hover:bg-emerald-500 hover:shadow-emerald-500/25 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 focus:ring-offset-gray-900">
                        <span class="absolute inset-y-0 left-0 flex items-center pl-3">
                            <svg class="h-5 w-5 text-emerald-300 group-hover:text-emerald-100 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
