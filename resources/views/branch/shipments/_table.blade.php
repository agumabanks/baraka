@forelse($shipments as $shipment)
    <tr class="hover:bg-white/5 transition" data-shipment-id="{{ $shipment->id }}">
        ...
        <td class="px-4 py-3">
            <a href="#" onclick="showShipmentModal({{ $shipment->id }}); return false;" class="font-mono text-sm font-semibold text-emerald-400 hover:text-emerald-300">
        ...
        <td class="px-4 py-3">
            <div class="text-sm font-semibold">{{ $shipment->customerProfile?->contact_person ?? $shipment->customer?->name ?? 'Walk-in' }}</div>
            <div class="text-xs text-zinc-500">{{ $shipment->customerProfile?->phone ?? $shipment->customer?->phone ?? '' }}</div>
        </td>
        ...
        <td class="px-4 py-3">
            @php
                $statusValue = is_object($shipment->current_status) ? strtolower($shipment->current_status->value ?? 'created') : strtolower($shipment->current_status ?? $shipment->status ?? 'created');
                $status = str_replace(' ', '_', $statusValue);
            @endphp
        ...
        <td class="px-4 py-3 text-right">
            <div class="action-dropdown">
                ...
                <div class="action-dropdown-content">
                    ...
                    <a href="{{ route('branch.shipments.edit', $shipment) }}">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                        Edit Shipment
                    </a>
                    <div class="divider"></div>
                    <a href="{{ route('branch.pos.label', $shipment) }}" target="_blank">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg>
                        Print Label
                    </a>
                    <a href="{{ route('branch.pos.receipt', $shipment) }}" target="_blank">
...
@empty
    <tr>
        <td colspan="8" class="px-4 py-12 text-center">
            ...
            <a href="{{ route('branch.pos.index') }}" class="text-emerald-400 hover:text-emerald-300 text-sm">Create your first shipment</a>
        </td>
    </tr>
@endforelse
