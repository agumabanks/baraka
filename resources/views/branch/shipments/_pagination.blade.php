<div class="flex flex-col sm:flex-row items-center justify-between gap-4 px-4 py-3 border-t border-white/10">
    <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-2">
            ...
            <select id="perPageSelect" onchange="changePerPage(this.value)" class="bg-white/5 border border-white/10 rounded px-2 py-1 text-sm focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500">
                <option value="15" @selected(($perPage ?? 15) == 15)>15</option>
                <option value="25" @selected(($perPage ?? 15) == 25)>25</option>
                <option value="50" @selected(($perPage ?? 15) == 50)>50</option>
                <option value="100" @selected(($perPage ?? 15) == 100)>100</option>
            </select>
        </div>
    </div>
    ...
    @if($shipments->lastPage() > 1)
        <div class="flex items-center gap-2">
            ...
            @else
                <a href="{{ $shipments->previousPageUrl() }}&per_page={{ $perPage ?? 15 }}" class="pagination-link px-3 py-1.5 text-sm bg-white/5 hover:bg-white/10 rounded-lg transition-colors">Previous</a>
            @endif
            ...
            <div class="flex items-center gap-1">
                @foreach($shipments->getUrlRange(max(1, $shipments->currentPage() - 2), min($shipments->lastPage(), $shipments->currentPage() + 2)) as $page => $url)
                    ...
                    @else
                        <a href="{{ $url }}&per_page={{ $perPage ?? 15 }}" class="pagination-link px-3 py-1.5 text-sm bg-white/5 hover:bg-white/10 rounded-lg transition-colors">{{ $page }}</a>
                    @endif
                @endforeach
            </div>
            
            @if($shipments->hasMorePages())
                <a href="{{ $shipments->nextPageUrl() }}&per_page={{ $perPage ?? 15 }}" class="pagination-link px-3 py-1.5 text-sm bg-white/5 hover:bg-white/10 rounded-lg transition-colors">Next</a>
            @else
