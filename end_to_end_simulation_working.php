<?php
/**
 * Baraka Logistics Platform - Corrected End-to-End System Simulation
 * 
 * This script validates the complete shipment lifecycle using the actual database schema,
 * testing all system integrations with correct foreign key relationships.
 */
...
class BarakaSystemSimulation
{
    ...
    private function checkSystemHealth(): void
    {
        ...
        try {
            ...
            $tables = ['shipments', 'branches', 'webhook_endpoints', 'scan_events', 'edi_transactions', 'hubs'];
            $tableStatus = [];
    ...
    private function simulateBookingPhase(): array
    {
        ...
        try {
            ...
            // Create initial scan event using correct schema with hub ID
            $scanEventData = [
                'sscc' => $this->generateSSCC(),
                'type' => 'booked',
                'branch_id' => 1, // Use valid hub ID
                'user_id' => 1,
        ...
        try {
            ...
            foreach ($checkpoints as $checkpoint) {
                DB::table('scan_events')->insert([
                    ...
                    'branch_id' => 1, // Use valid hub ID for all checkpoints
                    'user_id' => 1,
            ...
            $edi856Data = [
                ...
                'reference' => $branchResult['tracking_number'],
    ...
    private function simulateDeliveryProcess(array $transferResult): array
    {
        ...
        try {
            ...
            $edi997Data = [
                ...
                'reference' => $transferResult['tracking_number'],
                'raw_document' => json_encode([
    ...
    private function testWebhookNotifications(array $bookingResult): array
    {
        ...
        try {
            ...
            foreach ($webhooks as $webhook) {
                // Update webhook trigger time to simulate successful delivery
